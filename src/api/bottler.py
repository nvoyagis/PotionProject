...
def post_deliver_bottles(potions_delivered: list[PotionInventory], order_id: int):
    ...
    with db.engine.begin() as connection:
        ...
        connection.execute(sqlalchemy.text("UPDATE resources SET green_ml = resources.green_ml - 50 * :sum"), [{"sum": teal_sum}])
        connection.execute(sqlalchemy.text("UPDATE resources SET blue_ml = resources.blue_ml - 50 * :sum"), [{"sum": teal_sum}])
